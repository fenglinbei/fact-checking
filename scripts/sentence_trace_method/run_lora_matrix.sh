...
PROMPT_EVIDENCE_PROMPT_TOKEN_BUDGET="${PROMPT_EVIDENCE_PROMPT_TOKEN_BUDGET:-}"
...
SFT_EARLY_STOPPING_METRIC="${SFT_EARLY_STOPPING_METRIC:-}"
...
prepare_lora_config() {
  ...
  if [[ -n "$SFT_EARLY_STOPPING_METRIC" ]]; then
    cmd+=(--early-stopping-metric "$SFT_EARLY_STOPPING_METRIC")
  fi
