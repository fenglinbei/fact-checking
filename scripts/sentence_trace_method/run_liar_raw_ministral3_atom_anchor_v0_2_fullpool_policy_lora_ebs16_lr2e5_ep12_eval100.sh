...
VERIFIER_BUILD_SPLITS="${VERIFIER_BUILD_SPLITS:-$MREC_SPLITS}"
...
selection_policy_requires_weight() {
  case "${EXPECTED_SELECTION_POLICY:-}" in
    learned_marginal_proxy|learned_marginal_reward|learned_marginal_one_shot) return 0 ;;
    *) return 1 ;;
  esac
}
...
printf '[atom-anchor-v0.2-fullpool-policy] MREC_POLICY_CONFIG=%s FINETUNE_MODE=%s BASE_CASE_NAME=%s MODEL_PATH=%s TRACE_BUILD_MODE=%s SOURCE_FEATURE_ROOT=%s TRACE_ROOT=%s TRACE_SHUFFLE_SOURCE_ROOT=%s TRACE_SHUFFLE_SEED=%s WEIGHT_FILE=%s TRACE_CANDIDATE_TOP_N=%s TRACE_MAX_STEPS=%s TRACE_MIN_STEPS=%s PROMPT_EVIDENCE_POLICY=%s PROMPT_EVIDENCE_MIN_COUNT=%s PROMPT_EVIDENCE_MAX_COUNT=%s PROMPT_EVIDENCE_TOKEN_BUDGET=%s PROMPT_EVIDENCE_PROMPT_TOKEN_BUDGET=%s EXPECTED_SELECTOR_NAME=%s QUALITY_AUDIT_MODE=%s EVAL_SPLITS=%s REQUIRE_PROMPT_INPUT_IDS=%s SFT_GRADIENT_ACCUMULATION_STEPS=%s SFT_LEARNING_RATE=%s SFT_NUM_TRAIN_EPOCHS=%s SFT_EVAL_STEPS=%s SFT_SAVE_STEPS=%s SFT_EARLY_STOPPING_PATIENCE=%s LORA_R=%s LORA_ALPHA=%s LORA_DROPOUT=%s RUNTIME_CACHE_ROOT=%s XDG_CACHE_HOME=%s VLLM_CACHE_ROOT=%s TORCHINDUCTOR_CACHE_DIR=%s TRITON_CACHE_DIR=%s MAP_ABLATION_MODE=%s\n' \
  "$MREC_POLICY_CONFIG" \
  "$FINETUNE_MODE" \
  "$BASE_CASE_NAME" \
  "$MODEL_PATH" \
...
export PROMPT_EVIDENCE_PROMPT_TOKEN_BUDGET
