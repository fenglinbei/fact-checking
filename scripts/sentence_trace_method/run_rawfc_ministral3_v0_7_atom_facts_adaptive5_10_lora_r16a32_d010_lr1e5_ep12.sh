...
export SELECTOR_NAME="${SELECTOR_NAME:-v0_7_atom_facts_budgeted_marginal_chain_adaptive5_10}"
...
export CASE_SUFFIX="${CASE_SUFFIX:-__v0_7_atom_facts_bm_adaptive5_10}"
...
ATOM_FACTS_SOURCE_ROOT="${ATOM_FACTS_SOURCE_ROOT:-outputs/selectors/evidence_chain_graph/rawfc_v0_7_atom_facts_budgeted_marginal_adaptive5_10}"
PREPARE_RAWFC_ATOM_FACTS_SOURCES="${PREPARE_RAWFC_ATOM_FACTS_SOURCES:-true}"
FORCE_ATOM_FACTS_STAGE="${FORCE_ATOM_FACTS_STAGE:-true}"
...
stage_force_args=()
if [[ "${FORCE_ATOM_FACTS_STAGE}" == "true" || "${FORCE_ATOM_FACTS_STAGE}" == "1" ]]; then
  stage_force_args=(--force)
fi

if [[ "${PREPARE_RAWFC_ATOM_FACTS_SOURCES}" == "true" || "${PREPARE_RAWFC_ATOM_FACTS_SOURCES}" == "1" ]]; then
  run_cmd "$PYTHON_BIN" scripts/sentence_trace_method/stage_sources.py \
    --dataset rawfc \
    --output-root "$OUTPUT_ROOT" \
    --source-root "$ATOM_FACTS_SOURCE_ROOT" \
...
export PREPARE_RAWFC_ATOM_FACTS_SOURCES=false
