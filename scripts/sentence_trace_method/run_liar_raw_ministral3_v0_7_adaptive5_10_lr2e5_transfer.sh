...
export SWANLAB_PROJECT="${SWANLAB_PROJECT:-fact-checking-sentence-trace-method-liar-ministral-transfer}"
...
IFS=',' read -r -a lr_values <<< "${LR_VALUES:-$SFT_LEARNING_RATE}"
...
for raw_ebs in "${ebs_values[@]}"; do
  ...
  for raw_lr in "${lr_values[@]}"; do
    lr="${raw_lr// /}"
    [[ -z "$lr" ]] && continue
    lora_suffix="_lora_ebs${ebs}_lr$(lr_slug "$lr")_ep12_eval100_pat8_liarw"
    printf '\n[liar-ministral-transfer] DATASETS=%s MODELS=%s SELECTOR_NAME=%s CASE_SUFFIX=%s LORA_SUFFIX=%s EBS=%s DEEPSPEED_CONFIG=%s SFT_GRADIENT_ACCUMULATION_STEPS=%s SFT_LEARNING_RATE=%s SFT_NUM_TRAIN_EPOCHS=%s REQUIRE_PROMPT_INPUT_IDS=%s\n' \
      "$DATASETS" "$MODELS" "$SELECTOR_NAME" "$CASE_SUFFIX" "$lora_suffix" "$ebs" "$ds_config" "$ga" "$lr" "$SFT_NUM_TRAIN_EPOCHS" "$REQUIRE_PROMPT_INPUT_IDS"
    PYTHON_BIN="$PYTHON_BIN" \
