...
export SELECTOR_NAME="${SELECTOR_NAME:-v0_7_atom_facts_abc_tight_budgeted_marginal_chain_adaptive5_10}"
...
export CASE_SUFFIX="${CASE_SUFFIX:-__v0_7_atom_facts_abc_tight_bm_adaptive5_10__qec_min}"
...
ATOM_FACTS_ABC_SOURCE_ROOT="${ATOM_FACTS_ABC_SOURCE_ROOT:-outputs/selectors/evidence_chain_graph/rawfc_v0_7_atom_facts_abc_tight_budgeted_marginal_adaptive5_10}"
...
printf '\n[rawfc-ministral3-atom-facts-abc-tight-qec-min-lora] DATASETS=rawfc MODELS=ministral3_8b TRACE_PROMPT_STYLE=%s EVIDENCE_TEXT_MODE=%s CASE_SUFFIX=%s LORA_SUFFIX=%s SELECTOR_NAME=%s SOURCE_ROOT=%s EBS=16 DEEPSPEED_CONFIG=configs/deepspeed_zero2_bsz1_ga4.json SFT_GRADIENT_ACCUMULATION_STEPS=4 SFT_LEARNING_RATE=%s SFT_NUM_TRAIN_EPOCHS=%s SFT_EVAL_STEPS=%s SFT_SAVE_STEPS=%s SFT_EARLY_STOPPING_PATIENCE=%s SFT_EARLY_STOPPING_METRIC=%s REQUIRE_PROMPT_INPUT_IDS=true TEST_EVAL=%s/%s RUN_TAU_EVAL=%s\n' \
  "$TRACE_PROMPT_STYLE" "$EVIDENCE_TEXT_MODE" "$CASE_SUFFIX" "$LORA_SUFFIX" "$SELECTOR_NAME" "$ATOM_FACTS_ABC_SOURCE_ROOT" "$SFT_LEARNING_RATE" "$SFT_NUM_TRAIN_EPOCHS" "$SFT_EVAL_STEPS" "$SFT_SAVE_STEPS" "$SFT_EARLY_STOPPING_PATIENCE" "$SFT_EARLY_STOPPING_METRIC" "$TEST_SPLIT" "$TEST_CHECKPOINT" "$RUN_TAU_EVAL"
