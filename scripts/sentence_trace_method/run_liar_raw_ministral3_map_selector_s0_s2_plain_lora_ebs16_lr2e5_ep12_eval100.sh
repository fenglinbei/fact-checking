...
SELECTORS="${MAP_SELECTOR_CASES:-map_selector_s0_retrieval_top5 map_selector_s1_mmr_pool_top5 map_selector_s2_map_quality_top5}"
...
if [[ "$PREPARE_MAP_SELECTOR_TRACES" == "true" || "$PREPARE_MAP_SELECTOR_TRACES" == "1" ]]; then
  PYTHON_BIN="$PYTHON_BIN" \
    ...
    bash "${ROOT_DIR}/scripts/phase5_selectors/run/run_liar_raw_map_selector_ablation_s0_s2.sh"
fi

printf '[liar-raw-ministral3-map-selector-s0-s2-plain] DATASETS=%s MODELS=%s MODE=%s EVAL_SPLITS=%s RUN_TAU_EVAL=%s TRACE_PROMPT_STYLE=%s FORCE_STAGE=%s SELECTORS=%s LORA_SUFFIX=%s EXPECTED_CHUNK_MMR_FINGERPRINT=%s\n' \
...
for selector in ${SELECTORS}; do
  ...
  printf '\n[liar-raw-ministral3-map-selector-plain] SELECTOR_NAME=%s SOURCE_ROOT=%s CASE_SUFFIX=%s TRACE_PROMPT_STYLE=%s EVAL_SPLITS=%s RUN_TAU_EVAL=%s FORCE_STAGE=%s SFT_LEARNING_RATE=%s SFT_NUM_TRAIN_EPOCHS=%s SFT_EVAL_STEPS=%s REQUIRE_PROMPT_INPUT_IDS=%s LIAR_CLASS_WEIGHTS=%s\n' \
    "$SELECTOR_NAME" "$SOURCE_ROOT" "$CASE_SUFFIX" "$TRACE_PROMPT_STYLE" "$EVAL_SPLITS" "$RUN_TAU_EVAL" "$FORCE_STAGE" "$SFT_LEARNING_RATE" "$SFT_NUM_TRAIN_EPOCHS" "$SFT_EVAL_STEPS" "$REQUIRE_PROMPT_INPUT_IDS" "$LIAR_CLASS_WEIGHTS"
