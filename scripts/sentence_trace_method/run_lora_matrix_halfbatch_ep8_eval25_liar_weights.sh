...
# - evaluate and checkpoint every 25 optimizer updates
# - apply LIAR-only label-token CE class weights
export LORA_SUFFIX="${LORA_SUFFIX:-_lora_halfbatch_ep8_eval25_liarw}"
...
export SFT_EVAL_STEPS="${SFT_EVAL_STEPS:-25}"
...
printf '[lora-tuned] gradient_accumulation_steps=%s num_train_epochs=%s eval_steps=%s save_steps=%s\n' \
  "$SFT_GRADIENT_ACCUMULATION_STEPS" "$SFT_NUM_TRAIN_EPOCHS" "$SFT_EVAL_STEPS" "$SFT_SAVE_STEPS"
