"""Train Step3 set-aware listwise evidence selector from Stage2 oracle rows."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a set-aware listwise Stage2 evidence selector.")
    ...
    p.add_argument("--mask-weight", type=float, default=0.3)
    p.add_argument("--listmle-weight", type=float, default=1.0)
...
def _validate_and_maybe_save(
    ...
    *,
    ...
    if force_save or metric_value > best_metric + 1e-8:
        ...
        metadata = {
            "selector_type": "set_aware_listwise",
            ...
            "losses": {
                "selected_mask_bce": float(args.mask_weight),
...
def _build_optimizer(model: SetAwareListwiseSelectorModel, args: argparse.Namespace) -> torch.optim.Optimizer:
...
def _maybe_permute_batch(
    ...
    *,
    ...
    if probability <= 0.0:
        return batch
    out: list[Stage2OracleExample] = []
    for example in batch:
        if random.random() >= probability or len(example.candidates) <= 1:
            out.append(example)
            continue
        perm = list(range(len(example.candidates)))
        random.shuffle(perm)
        inverse = {old: new for new, old in enumerate(perm)}
        selected = [inverse[idx] for idx in example.selected_indices if idx in inverse]
        candidates = [dict(example.candidates[idx]) for idx in perm]
        candidate_scores = [dict(example.candidate_scores[idx]) for idx in perm]
        for new_idx, row in enumerate(candidate_scores):
            row.setdefault("original_candidate_idx", int(row.get("candidate_idx", perm[new_idx])))
            row["candidate_idx"] = int(new_idx)
        out.append(
            replace(
                example,
                candidates=candidates[:max_candidates],
                candidate_scores=candidate_scores[:max_candidates],
