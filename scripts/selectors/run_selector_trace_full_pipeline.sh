...
#
# Examples:
#   DRY_RUN=true bash scripts/selectors/run_selector_trace_full_pipeline.sh
#
...
#   bash scripts/selectors/run_selector_trace_full_pipeline.sh
#
...
build_case() {
  ...
  if [[ "${DRY_RUN}" == "true" ]]; then
    printf '[selector-trace-full] dry-run build: python scripts/selectors/build_trace_verifier_data.py'
    printf ' %q' "${args[@]}"
    printf '\n'
    return 0
  fi
  python scripts/selectors/build_trace_verifier_data.py "${args[@]}"
}
