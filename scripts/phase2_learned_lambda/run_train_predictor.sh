...
#
# Usage:
#   bash scripts/phase2_learned_lambda/run_train_predictor.sh
#   SPLIT_NAME=val bash scripts/phase2_learned_lambda/run_train_predictor.sh
#   ORACLE_LAMBDAS=outputs/learned_lambda/oracle_lambda_train.jsonl bash scripts/phase2_learned_lambda/run_train_predictor.sh
#   CHUNK_MMR_CACHE=outputs/cache/chunk_mmr/<fingerprint>/train.pkl bash scripts/phase2_learned_lambda/run_train_predictor.sh
#   EPOCHS=100 BATCH_SIZE=128 PROGRESS=false bash scripts/phase2_learned_lambda/run_train_predictor.sh
#   CANDIDATE_TOP_K=16 bash scripts/phase2_learned_lambda/run_train_predictor.sh  # optional truncation; default uses full chunk pool
#   OBJECTIVE=classification OUTPUT_DIR=outputs/learned_lambda_cls bash scripts/phase2_learned_lambda/run_train_predictor.sh
#
# Extra CLI args are forwarded to train_predictor.py, for example:
#   bash scripts/phase2_learned_lambda/run_train_predictor.sh --lr 5e-4 --patience 20
...
if [[ ! -f "${ORACLE_LAMBDAS}" ]]; then
  ...
  echo "[run_train_predictor] Run scripts/phase2_learned_lambda/run_compute_oracle_lambda.sh first, or set ORACLE_LAMBDAS." >&2
  exit 1
fi
