...
#
# Usage:
#   bash scripts/phase2_learned_lambda/run_train_predictor_high_margin.sh
#   MARGIN_THRESHOLD=0.03 bash scripts/phase2_learned_lambda/run_train_predictor_high_margin.sh  # alternate threshold
#
# Extra CLI args are forwarded to train_predictor.py, for example:
#   bash scripts/phase2_learned_lambda/run_train_predictor_high_margin.sh --lr 5e-4
...
if [[ ! -f "${ORACLE_LAMBDAS}" ]]; then
  ...
  echo "[run_train_predictor_high_margin] Run scripts/phase2_learned_lambda/run_compute_oracle_lambda.sh first, or set ORACLE_LAMBDAS." >&2
  exit 1
fi
