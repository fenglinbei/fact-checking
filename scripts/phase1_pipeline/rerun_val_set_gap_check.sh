...
#
# Usage:
#   bash scripts/phase1_pipeline/rerun_val_set_gap_check.sh
#   DRY_RUN=true bash scripts/phase1_pipeline/rerun_val_set_gap_check.sh
#   B3_TOP_KS="3,6" MMR_1024_LAMBDAS="0.4,1.0" bash scripts/phase1_pipeline/rerun_val_set_gap_check.sh
#   RUN_B3=false RUN_MMR_1024=true RUN_MMR_2048=false bash scripts/phase1_pipeline/rerun_val_set_gap_check.sh
#   SKIP_MISSING=false bash scripts/phase1_pipeline/rerun_val_set_gap_check.sh
...
if [[ "${WRITE_SUMMARY}" == "true" && "${DRY_RUN}" != "true" ]]; then
  ...
  python scripts/phase1_pipeline/summarize_infer_metrics.py \
    "${B3_ROOT}" \
    "${MMR_1024_ROOT}" \
    "${MMR_2048_ROOT}" \
