...
#
# 用法:
#   bash scripts/phase1_pipeline/run_mmr_topk_sweep_infer.sh
#   TOP_KS="0,2,4" bash scripts/phase1_pipeline/run_mmr_topk_sweep_infer.sh
...
for top_k_index in "${!CLEAN_TOP_K_ARRAY[@]}"; do
  ...
  python scripts/phase1_pipeline/mmr_topk_reuse.py prepare-config \
  ...
  python scripts/phase1_pipeline/mmr_topk_reuse.py summarize \
    --top-k "${top_k}" \
    --topk-run-dir "${TOPK_RUN_DIR}" \
