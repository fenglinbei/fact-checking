...
#
# Default use:
#   bash scripts/phase4_verifier/run_oracle_direct_vllm_server.sh
#
# Common overrides:
...
#
# Background mode:
#   BACKGROUND=true WAIT_READY=true bash scripts/phase4_verifier/run_oracle_direct_vllm_server.sh
#   ACTION=status bash scripts/phase4_verifier/run_oracle_direct_vllm_server.sh
#   ACTION=stop bash scripts/phase4_verifier/run_oracle_direct_vllm_server.sh
#
...
case "${SERVE_MODE}" in
  merged)
    ...
    MODEL_PATH="$(
      PYTHONPATH=src python scripts/phase4_verifier/prepare_lora_vllm_model.py \
