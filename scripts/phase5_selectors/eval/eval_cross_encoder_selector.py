"""Selection-only eval for the Step1 cross-encoder evidence selector."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate cross-encoder selector against Stage2 oracle order.")
...
def main() -> None:
    ...
    for example in tqdm(
        examples,
        desc=f"cross-encoder selector eval [{args.split}]",
        unit="claim",
        dynamic_ncols=True,
        disable=args.no_progress,
    ):
        scores = selector.score(example.claim, [candidate_text(candidate) for candidate in example.candidates])
    ...
    print(
        "Cross-encoder Recall@5={rec:.4f}, Jaccard@5={jac:.4f}, "
