...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build question-decomposition retrieval traces from generated questions.")
    p.add_argument("--questions-jsonl", required=True)
    ...
    p.add_argument("--per-question-keep", type=int, default=20)
    ...
    p.add_argument("--q1-weight", type=float, default=1.2)
    p.add_argument("--other-question-weight", type=float, default=1.0)
...
def main() -> None:
    ...
    question_rows = read_jsonl(args.questions_jsonl)
    if args.sample_limit is not None:
        question_rows = question_rows[: int(args.sample_limit)]
    if not question_rows:
        raise ValueError("No question rows loaded.")

    chunk_samples = _load_chunk_samples(args.chunk_cache_path)
    pairs = align_questions_and_chunks(question_rows, chunk_samples)
    ...
    question_embeddings = _encode_questions(question_rows, embedder, disable_progress=bool(args.no_progress))
    params = RetrievalParams(
        per_question_keep=int(args.per_question_keep),
        merged_pool_size=int(args.merged_pool_size),
        selector_top_k=int(args.selector_top_k),
        rrf_k=float(args.rrf_k),
        q1_weight=float(args.q1_weight),
        other_question_weight=float(args.other_question_weight),
    ...
    iterator = tqdm(pairs, desc="question retrieval", unit="claim", dynamic_ncols=True, disable=bool(args.no_progress))
    for question_row, sample in iterator:
        qd_row = build_question_decomp_retrieval_row(
            question_row,
            sample,
        ...
        _add_baseline_overlap(qd_row, baseline_row)
    ...
    print(
        "question_decomp oracle_pool_recall@15={:.4f} selected_recall@5={:.4f} jaccard@5={:.4f}".format(
            metrics["question_decomp"]["oracle_pool_recall@15"],
            metrics["question_decomp"]["oracle_selected_recall@5"],
            metrics["question_decomp"]["jaccard@5"],
        )
    )
    print(
...
def _encode_questions(
    ...
    *,
    ...
    for row in question_rows:
        ...
        for idx, question in enumerate(row.get("questions") or [], start=1):
            question_id = str(question.get("id") or f"q{idx}")
            keys.append((event_id, question_id))
            texts.append(str(question.get("question") or ""))
    if not texts:
        return {}
    if not disable_progress:
        print(f"[question-retrieval] Encoding {len(texts)} question(s)")
    embeddings = embedder.encode(texts, is_query=True)
    return {key: embeddings[idx] for idx, key in enumerate(keys)}


def _add_baseline_overlap(qd_row: dict[str, Any], baseline_row: dict[str, Any]) -> None:
    baseline = {
        canonicalize_sentence(str(candidate.get("text") or ""))
        for candidate in baseline_row.get("candidates") or []
    }
    overlap: list[str] = []
    for candidate in qd_row.get("selected_evidence") or []:
        text = canonicalize_sentence(str(candidate.get("text") or ""))
        if text and text in baseline:
            overlap.append(text)
    qd_row["baseline_top5_overlap"] = overlap
