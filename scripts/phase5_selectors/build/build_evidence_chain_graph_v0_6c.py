...
DEFAULT_OUTPUT_DIR = "outputs/selectors/evidence_chain_graph/v0_6c_adaptive5_10_val"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build v0.6c rule-step adaptive evidence-chain graph artifacts.")
...
def main() -> None:
    ...
    params = RuleStepEvidenceChainParams(
    ...
    graph_rows = [build_rule_step_evidence_chain_graph_row(row, params=params) for row in rows]
    traces = [row.get("selection_trace") or {} for row in graph_rows]
    diagnostics = summarize_rule_step_chain_graph_rows(graph_rows)
