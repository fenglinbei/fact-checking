...
QUESTION_OUTPUT_ROOT="${QUESTION_OUTPUT_ROOT:-outputs/selectors/question_decomp_retrieval/rawfc_deepseek_v0_abc}"
EVIDENCE_MAP_ROOT="${EVIDENCE_MAP_ROOT:-outputs/selectors/evidence_map_selector/rawfc_v0_7_atom_facts_abc}"
GRAPH_ROOT="${GRAPH_ROOT:-outputs/selectors/evidence_chain_graph/rawfc_v0_7_atom_facts_abc_budgeted_marginal_adaptive5_10}"
...
raw_path_for_split() {
  ...
  if [[ "${split}" == "train" ]]; then
    ...
    echo "[rawfc-v0.7-atom-facts-abc] unsupported split: ${split}" >&2
    exit 2
  fi
}
...
echo "[rawfc-v0.7-atom-facts-abc] splits       : ${SPLITS}"
echo "[rawfc-v0.7-atom-facts-abc] config       : ${CONFIG}"
echo "[rawfc-v0.7-atom-facts-abc] fingerprint  : ${CHUNK_MMR_FINGERPRINT}"
echo "[rawfc-v0.7-atom-facts-abc] roots        : ${QUESTION_OUTPUT_ROOT} | ${EVIDENCE_MAP_ROOT} | ${GRAPH_ROOT}"
echo "[rawfc-v0.7-atom-facts-abc] stages       : cache=${RUN_CACHE_BUILD} qd=${RUN_QD} map=${RUN_EVIDENCE_MAP} graph=${RUN_GRAPH_BUILD}"
echo "[rawfc-v0.7-atom-facts-abc] prompt       : ${PROMPT_VERSION}"
...
for split in ${SPLITS}; do
  ...
  echo "[rawfc-v0.7-atom-facts-abc] split        : ${split}"
  echo "[rawfc-v0.7-atom-facts-abc] raw          : ${raw_path}"
  echo "[rawfc-v0.7-atom-facts-abc] qd union     : ${union_pool}"
  echo "[rawfc-v0.7-atom-facts-abc] evidence map : ${em_dir}"
  echo "[rawfc-v0.7-atom-facts-abc] graph        : ${graph_dir}"

  if [[ "${RUN_EVIDENCE_MAP}" == "true" || "${RUN_EVIDENCE_MAP}" == "1" ]]; then
    if [[ ! -s "${union_pool}" ]]; then
      echo "[rawfc-v0.7-atom-facts-abc] missing QD union pool: ${union_pool}" >&2
      echo "[rawfc-v0.7-atom-facts-abc] rerun with RUN_QD=true first" >&2
      exit 1
    fi
  ...
  if [[ "${RUN_GRAPH_BUILD}" == "true" || "${RUN_GRAPH_BUILD}" == "1" ]]; then
    if [[ ! -s "${features}" ]]; then
      echo "[rawfc-v0.7-atom-facts-abc] missing evidence-map features: ${features}" >&2
      exit 1
    fi
...
echo "[rawfc-v0.7-atom-facts-abc] done"
