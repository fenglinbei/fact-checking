...
EVIDENCE_MAP_ROOT="${EVIDENCE_MAP_ROOT:-outputs/selectors/evidence_map_selector/liar_raw_v0_7_atom_facts_abc}"
GRAPH_ROOT="${GRAPH_ROOT:-outputs/selectors/evidence_chain_graph/liar_raw_v0_7_atom_facts_abc_budgeted_marginal_adaptive5_10}"

RUN_CACHE_BUILD="${RUN_CACHE_BUILD:-true}"
RUN_QD="${RUN_QD:-true}"
RUN_EVIDENCE_MAP="${RUN_EVIDENCE_MAP:-true}"
RUN_GRAPH_BUILD="${RUN_GRAPH_BUILD:-true}"
RUN_STAGE_SOURCES="${RUN_STAGE_SOURCES:-true}"
...
raw_path_for_split() {
  ...
  if [[ "${split}" == "train" ]]; then
    ...
    echo "[liar-raw-v0.7-atom-facts-abc] unsupported split: ${split}" >&2
    exit 2
  fi
}
...
graph_source_root() {
  if [[ -n "${SAMPLE_SUFFIX}" ]]; then
    printf "%s%s" "${GRAPH_ROOT}" "${SAMPLE_SUFFIX}"
  else
    printf "%s" "${GRAPH_ROOT}"
  fi
}
...
graph_dir_for_split() {
  local split="$1"
  if [[ -n "${SAMPLE_SUFFIX}" ]]; then
    printf "%s%s_%s" "${GRAPH_ROOT}" "${SAMPLE_SUFFIX}" "${split}"
  else
    printf "%s_%s" "${GRAPH_ROOT}" "${split}"
  fi
}

echo "[liar-raw-v0.7-atom-facts-abc] splits       : ${SPLITS}"
echo "[liar-raw-v0.7-atom-facts-abc] config       : ${CONFIG}"
echo "[liar-raw-v0.7-atom-facts-abc] fingerprint  : ${CHUNK_MMR_FINGERPRINT}"
echo "[liar-raw-v0.7-atom-facts-abc] roots        : ${QUESTION_OUTPUT_ROOT} | ${EVIDENCE_MAP_ROOT} | ${GRAPH_ROOT}"
echo "[liar-raw-v0.7-atom-facts-abc] stages       : cache=${RUN_CACHE_BUILD} qd=${RUN_QD} map=${RUN_EVIDENCE_MAP} graph=${RUN_GRAPH_BUILD} stage=${RUN_STAGE_SOURCES}"
echo "[liar-raw-v0.7-atom-facts-abc] prompt       : ${PROMPT_VERSION}"
...
if [[ "${RUN_QD}" == "true" || "${RUN_QD}" == "1" ]]; then
  ...
  CHUNK_MMR_FINGERPRINT="${CHUNK_MMR_FINGERPRINT}" \
  bash "${SCRIPT_DIR}/run_liar_raw_v0_7_atom_facts_abc_qd.sh"
fi
...
for split in ${SPLITS}; do
  ...
  echo "[liar-raw-v0.7-atom-facts-abc] split        : ${split}"
  echo "[liar-raw-v0.7-atom-facts-abc] raw          : ${raw_path}"
  echo "[liar-raw-v0.7-atom-facts-abc] qd union     : ${union_pool}"
  echo "[liar-raw-v0.7-atom-facts-abc] evidence map : ${em_dir}"
  echo "[liar-raw-v0.7-atom-facts-abc] graph        : ${graph_dir}"

  if [[ "${RUN_EVIDENCE_MAP}" == "true" || "${RUN_EVIDENCE_MAP}" == "1" ]]; then
    if [[ ! -s "${union_pool}" ]]; then
      echo "[liar-raw-v0.7-atom-facts-abc] missing QD union pool: ${union_pool}" >&2
      echo "[liar-raw-v0.7-atom-facts-abc] rerun with RUN_QD=true first" >&2
      exit 1
    fi
  ...
  if [[ "${RUN_GRAPH_BUILD}" == "true" || "${RUN_GRAPH_BUILD}" == "1" ]]; then
    if [[ ! -s "${features}" ]]; then
      echo "[liar-raw-v0.7-atom-facts-abc] missing evidence-map features: ${features}" >&2
      exit 1
    fi
...
if [[ "${RUN_STAGE_SOURCES}" == "true" || "${RUN_STAGE_SOURCES}" == "1" ]]; then
  PYTHON_BIN="${PYTHON_BIN}" \
  ...
  CHUNK_MMR_FINGERPRINT="${CHUNK_MMR_FINGERPRINT}" \
  bash "${SCRIPT_DIR}/run_liar_raw_v0_7_atom_facts_abc_stage_sources.sh"
fi

echo "[liar-raw-v0.7-atom-facts-abc] done"
