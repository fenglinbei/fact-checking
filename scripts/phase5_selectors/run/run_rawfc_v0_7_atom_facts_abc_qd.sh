...
QUESTION_CACHE_DIR="${QUESTION_CACHE_DIR:-outputs/selectors/question_decomp_retrieval/rawfc_question_cache_abc}"
...
raw_path_for_split() {
  ...
  if [[ "${split}" == "train" ]]; then
    ...
    echo "[rawfc-v0.7-atom-facts-abc-qd] unsupported split: ${split}" >&2
    exit 2
  fi
}
...
echo "[rawfc-v0.7-atom-facts-abc-qd] splits      : ${SPLITS}"
echo "[rawfc-v0.7-atom-facts-abc-qd] config      : ${CONFIG}"
echo "[rawfc-v0.7-atom-facts-abc-qd] output root : ${QUESTION_OUTPUT_ROOT}"
echo "[rawfc-v0.7-atom-facts-abc-qd] fingerprint : ${CHUNK_MMR_FINGERPRINT}"
echo "[rawfc-v0.7-atom-facts-abc-qd] stages      : ${RUN_QUESTION_CACHE}/${RUN_QD_RETRIEVAL}/${RUN_QD_UNION}"
...
for split in ${SPLITS}; do
  ...
  echo "[rawfc-v0.7-atom-facts-abc-qd] split       : ${split}"
  echo "[rawfc-v0.7-atom-facts-abc-qd] raw         : ${raw_path}"
  echo "[rawfc-v0.7-atom-facts-abc-qd] chunk cache : ${chunk_cache}"
  echo "[rawfc-v0.7-atom-facts-abc-qd] output      : ${qd_dir}"

  if [[ ! -s "${raw_path}" ]]; then
    echo "[rawfc-v0.7-atom-facts-abc-qd] missing RAWFC raw split: ${raw_path}" >&2
    exit 1
  fi
  mkdir -p "${qd_dir}"
  ...
  if [[ "${RUN_QUESTION_CACHE}" == "true" || "${RUN_QUESTION_CACHE}" == "1" ]]; then
    ...
    if [[ -z "${question_cache_id}" && "${MOCK_QUESTIONS}" == "true" ]]; then
      question_cache_id="rawfc3_abc_mock_${split}${SAMPLE_SUFFIX}"
    fi
  ...
  if [[ "${RUN_QD_RETRIEVAL}" == "true" || "${RUN_QD_RETRIEVAL}" == "1" ]]; then
    if [[ ! -s "${chunk_cache}" ]]; then
      echo "[rawfc-v0.7-atom-facts-abc-qd] missing ABC chunk cache: ${chunk_cache}" >&2
      echo "[rawfc-v0.7-atom-facts-abc-qd] run RUN_CACHE_BUILD=true first" >&2
      exit 1
    fi
    if [[ ! -s "${qd_dir}/questions_${split}.jsonl" ]]; then
      echo "[rawfc-v0.7-atom-facts-abc-qd] missing questions: ${qd_dir}/questions_${split}.jsonl" >&2
      exit 1
    fi
...
echo "[rawfc-v0.7-atom-facts-abc-qd] done"
