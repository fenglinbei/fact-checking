...
if [[ "${SKIP_DATASET}" != "true" ]]; then
  python scripts/phase5_selectors/build/build_pointwise_oracle_dataset.py \
...
if [[ "${SKIP_TRAIN}" != "true" ]]; then
  python scripts/phase5_selectors/train/train_pointwise_oracle_selector.py \
...
if [[ "${SKIP_SELECTION_EVAL}" != "true" ]]; then
  python scripts/phase5_selectors/eval/eval_pointwise_oracle_selector.py \
    --model-dir "${MODEL_DIR}" \
