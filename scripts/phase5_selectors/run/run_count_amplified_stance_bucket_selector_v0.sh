...
ANNOTATIONS="${OUTPUT_DIR}/deepseek_teacher_annotations_${SPLIT}.jsonl"
...
if [[ "${RUN_EVAL}" == "1" || "${RUN_EVAL}" == "true" || "${RUN_EVAL}" == "True" ]]; then
  for n in ${EVAL_N_BUCKETS}; do
    if [[ "${n}" == "3" ]]; then
      BUCKET_FILE="${OUTPUT_DIR}/candidate_stance_buckets_${SPLIT}.jsonl"
    else
      BUCKET_FILE="${OUTPUT_DIR}/candidate_stance_buckets_n${n}_${SPLIT}.jsonl"
    fi
