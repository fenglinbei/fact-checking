...
#
# Usage:
#   bash scripts/phase6_rl_mmr/run_sensitivity_gated.sh \
#       [STAGE_A_DIR]                        # default: outputs/rl_mmr/sensitivity_search
#       [EXPERIMENT]                         # default: mmr_sensitivity_gated
#       [MODE]                               # default: full
#
# Optional:
#   OUTPUT_SUBDIR=ts0p8_tr0p3_ll0p2_basic_eps0p05 bash scripts/phase6_rl_mmr/run_sensitivity_gated.sh
# ==============================================================================
set -euo pipefail
...
if [ ! -f "${STAGE_A_CSV}" ]; then
    ...
    echo "[stage-B][fatal] Run scripts/phase6_rl_mmr/run_stage_a_search.sh first." >&2
    exit 1
fi
