...
#   OBJECTIVE=soft_classification bash scripts/learned_lambda/run_train_predictor_coarse.sh
#   LAMBDA_GRID="0.1,0.5,0.9" bash scripts/learned_lambda/run_train_predictor_coarse.sh  # custom grid
#
# Extra CLI args are forwarded to train_predictor.py, for example:
#   bash scripts/learned_lambda/run_train_predictor_coarse.sh --softmax-temperature 3.0
...
if [[ ! -f "${ORACLE_LAMBDAS}" ]]; then
  ...
  echo "[run_train_predictor_coarse] Run scripts/learned_lambda/run_compute_oracle_lambda.sh first, or set ORACLE_LAMBDAS." >&2
  exit 1
fi
