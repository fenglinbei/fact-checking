...
除各方法段落中就地定义的符号（如 claim $c$、原子命题 $a_i$ 与其查询 $q_i$、证据单元 $u_j$、evidence map $M(u_j,a_i)$、atom 验证状态 $h_i^{(t)}\in\{U,S,R,Q,C\}$、证据链 $\mathcal{T}$ 等）外，本节仅集中说明贯穿全文、需统一指代的若干记号：$t$ 表示 selector 的构链步数，$\mathcal{T}_{<t}$ 为第 $t$ 步前已选证据序列；$\phi^{(t)}(u_j)\in\mathbb{R}^{12}$ 为候选 $u_j$ 在第 $t$ 步的边际特征向量（含解析增益 $\phi_{\mathrm{res}}$、信息增益 $\phi_{\mathrm{ent}}$、覆盖增益 $\phi_{\mathrm{cov}}$、新关系增益 $\phi_{\mathrm{new\text{-}rel}}$、立场张力 $\phi_{\mathrm{tension}}$、佐证增益 $\phi_{\mathrm{corr}}$、来源/文本新颖性 $\phi_{\mathrm{src\text{-}novel}},\phi_{\mathrm{text\text{-}novel}}$、map 置信 $\phi_{\mathrm{conf}}$、map 质量 $\phi_{\mathrm{map}}$、检索分数 $\phi_{\mathrm{ret}}$ 与长度代价 $\phi_{\mathrm{cost}}$），$g_{ij}^{(t)}$ 为候选对单个 atom 的解析增益，$\delta(d_{ij})$ 为由 directness 决定的递增权重；为避免歧义，selector 学习器参数记作 $\theta_{\mathrm{sel}}$（含 cost 项 $\theta_c$，经 softplus 映射为非负权重 $w_\ell,w_c$），verifier 模型参数记作 $\theta_{\mathrm{ver}}$，二者对应的边际效用与概率分别写作 $U_{\theta_{\mathrm{sel}}}$ 与 $p_{\theta_{\mathrm{ver}}}$；$\rho_t$ 为第 $t$ 步的 resolved atom rate，$\rho_{\mathrm{target}}$ 为其目标，$\tau_{\mathrm{stop}}$ 为效用停止阈值，$k_{\min},k_{\max}$ 为证据数上下界，$K^\ast$ 为 prompt evidence policy 截断后 verifier 可见的证据数，$\mathcal{T}_{\mathrm{prompt}}$ 为对应证据前缀。另需注意，selector 在训练时维护 atom 上的软状态分布 $p_i^{(t)}(s)$（atom $a_i$ 在第 $t$ 步处于状态 $s$ 的概率质量），推理时退化为硬状态 $h_i^{(t)}$。
...
$$h_i^{(t)}\in\{U,S,R,Q,C\},$$
分别表示 unresolved, supported, refuted, qualified, conflicted。初始时所有 atom 均为 $U$。需要说明的是，selector 在训练阶段维护的是 atom 上的软状态分布 $p_i^{(t)}(s)$（atom $a_i$ 在第 $t$ 步处于状态 $s\in\{U,S,R,Q,C\}$ 的概率质量），用以支持平滑的边际增益估计与梯度回传；推理时则退化为上述硬状态 $h_i^{(t)}$，即取概率最大的状态。状态转移遵循：support/refute/qualify 分别映射到 $S/R/Q$，若新证据与已有 $S/R$ 立场冲突则进入 $C$。
...
推理时，selector 进行贪心构链：
$$u_t = \arg\max_{u_j\in \mathcal R\setminus \mathcal T_{<t}} U_{\theta_{\mathrm{sel}}}(u_j\mid \mathcal T_{<t},H^{(t)}).$$
