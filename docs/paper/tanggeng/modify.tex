...
\end{equation}
其中 $E_{start}$、$E_{end}$ 和 $E_{sup}$ 分别表示目标知识点集合上的初始得分、最终得分和满分。进一步地，为了刻画知识点推荐层在目标推进过程中的阶段性增益，本文将学习收益具体化为目标集合上的两类归一化收益信号，即离散达成收益与连续掌握收益：
\begin{equation}
\begin{aligned}
E_N
&=
...
针对离散达成增益变化稀疏与阶段式高层决策调整不及时的问题，本文提出细粒度学习路径推荐框架 FGLP。其核心包括两项相互配合的设计：一是将高层知识点规划与低层习题执行同步到每次习题交互，使最新反馈能够直接作用于下一步知识点决策；二是融合离散达成增益与连续掌握增益，为逐步知识点规划提供细粒度过程监督。在此基础上，习题推荐层负责为当前知识点决策选择具有较高局部学习收益的具体习题。

\subsection{总体框架}
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=1.0\textwidth]{framework.pdf} % 换成你的图
  \caption{\textbf{Framework overview of FGLP.} (a) Exercise-synchronized fine-grained decision process. After each exercise interaction, FGLP updates the student state and replans the next knowledge point. (b) High-level knowledge point planning with discrete-continuous hybrid process rewards. (c) Target-conditioned exercise selection using state-aware two-tower matching.}
  \label{fig:framework}
\end{figure*}
如图~\ref{fig:framework} 所示，FGLP由学生状态估计模块 S-Agent、高层知识点规划器 L-Agent、低层习题选择器 P-Agent 与学生模拟器组成。图~\ref{fig:framework}(a)展示整体闭环：在时刻 $t$，S-Agent根据历史交互估计状态 $(h_t,p_t)$；L-Agent结合目标集合与剩余预算选择知识点 $g_t$；P-Agent在该知识点条件下选择习题 $e_t$；学生完成练习后，模拟器返回作答反馈 $y_t$ 并产生下一时刻状态。与阶段式分层方法不同，该闭环以一道习题为完整决策周期，因此高层规划器在每次交互后均可重新决策。

图~\ref{fig:framework}(b)展示高层知识点规划。L-Agent利用离散--连续混合过程奖励学习知识点策略，并根据目标完成情况分别执行目标推进与目标巩固。图~\ref{fig:framework}(c)展示低层习题选择。P-Agent通过状态感知的两塔匹配模型，对当前知识点条件下的候选习题进行评分，并学习选择具有更高局部学习收益的习题。

S-Agent与学生模拟器均采用预训练DIMKT模型，但二者参数相互独立。S-Agent用于根据可观测历史估计决策状态，学生模拟器则用于刻画环境中的状态转移，从而避免决策侧直接访问模拟器的内部状态。

\subsection{习题同步的细粒度分层决策}

\subsubsection{逐步知识点重规划与状态建模}

传统分层方法通常在阶段或子会话开始时确定高层目标，并在多次低层交互期间保持该目标不变。FGLP仍保留“知识点规划--习题执行”的职责分解，但不为高层目标预先分配固定练习窗口。每完成一道习题，更新后的学生状态都会立即返回L-Agent，由其重新选择下一步知识点：
\begin{equation}
(h_t,p_t,b_t)
\xrightarrow{\pi^h}
g_t
...
\end{equation}
因此，当 $g_{t+1}=g_t$ 时，系统继续练习当前知识点；当 $g_{t+1}\neq g_t$ 时，系统切换学习知识点；当 $g_{t+1}$ 等于更早时刻学习过的知识点时，系统实现知识点回访。三种行为均由同一高层策略根据最新状态自主决定，而非由预设阶段长度或掌握阈值直接规定。
...
\end{aligned}
\end{equation}
其中，$FC_{\theta_m}(\cdot)$ 和 $FC_{\phi_m}(\cdot)$ 分别表示策略侧与价值侧的前馈神经网络。策略网络输出当前状态下各知识点的选择分布，价值网络则估计该状态对应的长期回报。双阶段机制只改变不同学习阶段的优化重点，并不改变上述习题同步的决策周期；无论处于哪个阶段，L-Agent都在每道习题后重新选择知识点。

\subsection{离散--连续混合过程奖励}

\subsubsection{离散达成增益与连续掌握增益}

仅使用离散达成收益作为过程奖励时，奖励只有在目标知识点跨越掌握阈值后才会变化，因而无法反映未达阈值时的渐进改善以及达标后的进一步提升。为此，FGLP联合使用离散达成收益 $E_t^{N}$ 与连续掌握收益 $E_t^{S}$。前者保持对目标完成数量的直接关注，后者补充目标集合掌握状态的细粒度变化。对应的一步增益分别记为
\begin{equation}
\Delta E_t^{N}=E_{t+1}^{N}-E_t^{N},
\qquad
\Delta E_t^{S}=E_{t+1}^{S}-E_t^{S}.
\end{equation}
二者共同回答不同问题：$\Delta E_t^{N}$判断当前交互是否推动目标知识点达标，$\Delta E_t^{S}$则判断当前交互是否产生了尚未体现为阈值跨越的有效学习进展。

\subsubsection{双阶段混合奖励}

为了使双阶段策略与其优化目标保持一致，我们分别构造目标推进与目标巩固阶段的过程奖励。阶段一的一步奖励写为
\begin{equation}
r_t^{h,1}
=
\Delta E_t^{N}
...
\xi_t,
\end{equation}
其中，$\eta$为固定步级惩罚，$\xi_t$表示预算耗尽但目标尚未完成时的终局惩罚，其幅度为 $\xi=\lambda_{\xi}(1-E_T^{N})$。混合增益为策略提供逐步学习信号，步级惩罚与终局惩罚则促使策略在有限预算内提高目标推进效率。
...
\eta,
\end{equation}
其中，$\mu$为退化惩罚系数。阶段二主要利用连续掌握增益推动目标状态进一步提升；一旦某个目标知识点重新跌破阈值，退化惩罚将抑制可能破坏已达成目标的决策。

\subsubsection{高层策略优化}
...
\subsection{目标条件下的习题选择}
L-Agent决定下一步学习哪个知识点，P-Agent则负责高质量执行该知识点决策。给定当前知识点 $g_t$ 后，P-Agent回答“当前练习哪一道习题最有利于提升该知识点的掌握水平”。本文采用图~\ref{fig:framework}(c)所示的两塔结构学习学生与习题之间的匹配函数，并使用一步前瞻收益监督该局部选题过程。
...
在推理阶段，我们不再使用上述一步前瞻收益来重排候选题目，而是直接依据P-Agent已经学得的两塔匹配在合法候选集内完成选题。由此，混合过程奖励为L-Agent提供逐步知识点规划依据，习题同步架构使其能够在每次交互后立即调整规划，而P-Agent则负责选择习题执行当前知识点决策，三者共同形成细粒度闭环。
\begin{table}[H]
\centering
