...
\begin{quote}
\emph{Can a typed claim--evidence map provide label- and verifier-independent supervision for constructing an atom-indexed, source-linked evidence trace before verdict prediction?}
\end{quote}
...
\begin{enumerate}
\item 提出一种 typed、source-linked 的中间表示，将 claim atoms、candidate--atom alignments、稳定 report/chunk identity 与逐步 map-induced state transition 组织为可审计的 ordered evidence trace。
...
\item 通过独立人工双标与第三人盲化仲裁，验证本研究审计样本中的自动 claim atomization；三项主要质量维度的最终 gold 通过率均不低于 95.72\%，为在该审计范围内将其作为后续 retrieval 与 Evidence Map 构建的可靠上游结构输入提供直接人工证据。
\item 在 LIAR-RAW、RAWFC 与 SciFact 上评估该方法，并通过 LIAR-RAW two-seed fixed-$K$ matched-verifier crossover 分析 one-shot 与 state-conditioned evidence organization 对 verifier 与训练种子的方向敏感性。
\end{enumerate}

\section{Related Work}

\subsection{Evidence-grounded Fact Verification}
...
整体方法及监督边界如图~\ref{fig:framework} 所示。
...
\textbf{Claim Atomization and Query Rendering.}
...
\textbf{Claim-aware Evidence Chunking.}
...
\textbf{Atom-Union Candidate Pool Construction.}
...
\begin{aligned}
s(q_i,u_j)
={}&\beta_1\,\operatorname{norm}(\cos(e_{q_i},e_{u_j}))\\
...
\subsection{Structure-Induced Evidence Organization}

\textbf{Typed Evidence Map and Source Linkage.}
...
其中 $d_{ij}$ 是有序 directness，$c_{ij}\in[0,1]$ 是 map annotator 的自评置信度。Map 同时引用 $p(u_j)$，使每个 alignment 能回到原始 report 和 span。
...
每个 atom 的 map-induced state 记为 $h_i^{(t)}$，其取值属于 $\{U,S,R,Q,C\}$；整体状态为 $H^{(t)}=\{h_i^{(t)}\}_{i=1}^{m}$。这些状态描述 evidence acquisition progress，分别记录尚未覆盖、观察到 support、refute、qualify 或相互冲突的 map relations。

\textbf{Structure-induced Preferences.}
...
\textbf{Structure-induced State-conditioned Selector.}
...
这些 feature groups 分别描述当前结构状态的推进、新 atom/relation 信息、来源与文本新颖性、map annotation quality、检索相关性和上下文成本。实现中的 12 个标量特征为 \texttt{\detokenize{resolution_delta}}、\texttt{\detokenize{entropy_reduction}}、\texttt{\detokenize{new_atom_coverage}}、\texttt{\detokenize{new_relation_for_atom}}、\texttt{\detokenize{stance_tension}}、\texttt{\detokenize{corroboration_gain}}、\texttt{\detokenize{source_novelty}}、\texttt{\detokenize{text_novelty}}、\texttt{\detokenize{map_confidence}}、\texttt{\detokenize{map_quality}}、\texttt{\detokenize{retrieval_score}} 与 \texttt{\detokenize{cost_ratio}}。前十一项以非负权重进入 structural marginal score，cost 以惩罚项进入：
...
\textbf{State-Conditioned Full-Pool-Access Evidence Ordering.}
...
选中 $u_t$ 后，系统更新 atom state、已见 relation、source coverage 与 novelty statistics，再重新计算下一步分数。运行配置设置 \texttt{\detokenize{candidate_top_n=0}}、\texttt{\detokenize{max_steps=0}}，并将 absolute-score threshold 设为极低值；因此 selector 可以访问完整候选池，不会因 atom coverage threshold、固定 trace length 或分数阈值提前停止。实现仍会跳过规范化后重复或无效的候选，因此输出长度 $L$ 满足 $L\le |\mathcal C|$，本文将其称为 \textbf{full-pool-access audit ordering}，而不是候选池的严格全排列。该设计与在 retrieval rollout 内依据 formal proof state 或 early-sufficiency objective 动态终止的方法不同 \citep{Aly2022NaturalLogic,Alt2026EvidenceRanking}；本文把可见容量决策留给后续 prefix policy。
...
\subsection{Downstream Prefix Projection and Verification}

\textbf{Verifier-visible Prefix Policy.}

Prefix policy 只沿 audit ordering 选择前缀，\textbf{不重新打分、不重新排序}。对 LIAR-RAW 与 RAWFC，主策略为 minmax$(5,10)$：至少尝试保留五条 evidence units；达到 $k_{\min}=5$ 后，若 map-induced covered-atom rate 达到 $\rho_{\mathrm{target}}=1.0$，则在当前位置投影前缀，否则继续到最多十条。对 SciFact，主配置为 minmax$(9,9)$。令 $L=|\mathcal T^{\mathrm{audit}}|$、$k_0=\min(k_{\min},L)$、$k_1=\min(k_{\max},L)$，先由结构策略确定目标长度：
...
\[
\begin{aligned}
x_k&:=\operatorname{Render}
\bigl(c,\mathcal A,\mathcal T_{1:k}^{\mathrm{audit}}\bigr),\\
K^\ast&=\max\Bigl\{k\in\{0,\ldots,\widetilde K\}:
\operatorname{FitsContext}(x_k)\Bigr\}.
\end{aligned}
\]
...
\textbf{End-to-end Algorithm.}

\begin{figure*}[t]
\refstepcounter{paperalgorithm}
\label{alg:full-pool-ordering}
\begin{center}
\textbf{Algorithm \thepaperalgorithm: Structure-Induced Full-Pool-Access Evidence Ordering}
\end{center}
\begin{lstlisting}
...
\textbf{Verifier Rendering and Prediction.}
...
\textbf{Training and Inference Pipeline.}

\textbf{Structure-only selector training.} 对训练集依次生成 atoms、Atom-Union pool 和 Evidence Map；从每个 state 产生 winner-vs-rest pairs 并训练 selector weights，validation target 是 structure-winner pair accuracy。训练过程不启动 verifier，也不读取事实标签。

\textbf{Verifier training.} 固定 selector 后，为训练样本构建 audit ordering，再通过与推理阶段相同的 prefix policy 与 renderer 生成 verifier rows。只有该阶段读取 gold fact-checking labels。Selector weights 在 verifier training 中保持冻结。

\textbf{Inference.} 对 validation/test claim 执行相同的 atomization、retrieval、map annotation、full-pool-access ordering、prefix projection 与 label-token prediction。上游 procedure/configuration 与已训练参数在 split 之间固定，各 split 独立生成样本级 atoms、candidate pools 与 maps；verdict labels 不回流到任何上游模块。
...
\begin{itemize}
\item \textbf{RQ1：} structure-only 方法在 LIAR-RAW 与 RAWFC 上相对于可核实的同任务基线表现如何？
\item \textbf{RQ2：} 相同的结构监督边界能否适用于标签体系和证据来源均不同的 SciFact？
\item \textbf{RQ3：} 在这一选择器流程中，主要收益来自什么组件？
\item \textbf{RQ4：} Map标注能够多大程度影响最终结果？
\item \textbf{RQ5：} 在固定 evidence ordering 的条件下，证据容量与 verifier-visible prefix policy 如何影响判别性能和上下文成本？
\end{itemize}

\subsection{Datasets and Evaluation Protocol}

\subsubsection{LIAR-RAW}
...
SciFact 包含 1.4K expert-written claims、5,183 篇 scientific abstracts，以及 abstract-level stance 与 sentence-level rationale annotations \citep{Wadden2020SciFact}。本文使用 original 300-claim development split 和完整 5,183-abstract corpus，不使用 gold \texttt{\detokenize{cited_doc_ids}} 或 gold rationale 构造候选池。我们使用官方 full-pipeline scorer 报告 sentence Selection-only、sentence Selection+Label、abstract Label-only 与 abstract Label+Rationale 四项 micro-F1。Verifier 在 SciFact training split 上训练，development split 同时用于 checkpoint selection，因此结果只用于评估该方法在 \textbf{dataset-specific training} 下对 scientific fact-checking setting 的适用性，不作为 zero-shot transfer、hidden-test 或整体 SciFact SOTA 声明。

\subsection{Implementation Details}

\textbf{LLM-based structural annotation.} 结构标注调用配置如下：
...
\caption{LLM-based structural annotation settings.}
\label{tab:annotation-settings}
\begin{tabularx}{\textwidth}{@{}l l Y c c c r@{}}
\toprule
\textbf{Component} & \textbf{Dataset} & \textbf{API model string} & \textbf{Temperature} & \textbf{Top-$p$} & \textbf{Thinking} & \textbf{Max output tokens} \\
\midrule
Claim atomization & All & \texttt{\detokenize{deepseek-v4-flash}} & 0 & 1.0 & disabled & 2,048 \\
Evidence Map & LIAR-RAW / RAWFC & \texttt{\detokenize{deepseek-v4-flash}} & 0 & 1.0 & disabled & 2,048 \\
Evidence Map & SciFact & \texttt{\detokenize{deepseek-v4-flash}} & 0 & 1.0 & disabled & 4,096 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\textbf{Retrieval and selector.} LIAR-RAW/RAWFC 使用 BGE-base-en-v1.5 retrieval 与 Atom-Union candidate pool；SciFact 使用相同 dense encoder 的 open-corpus retrieval。Preference training 在每个样本的 top-20 candidates 上执行五步 rollout，训练 30 epochs，learning rate 为 0.05，并从 equal-weight neutral initialization 开始；推理阶段的 audit ordering 则访问通过去重与有效性检查的完整候选池。

\textbf{Verifier and context policy.} 三个数据集均使用 Ministral-3-8B-Instruct-2512 \citep{MistralAI2025Ministral3} 和 LoRA verifier。LIAR-RAW/RAWFC 使用 minmax$(5,10)$ prefix policy，SciFact 使用 minmax$(9,9)$；renderer 采用 \texttt{\detokenize{mrec_min}} style，完整输入最长 1,024 tokens。Verifier 通过候选 label tokens 的 logits 作确定性判定，不使用生成式 sampling temperature 或 thinking mode。共同设置为四卡 ZeRO-2、BF16、per-device batch size 1、gradient accumulation 4（effective batch size 16）、AdamW、weight decay 0.01、warmup ratio 0.03、cosine-with-restarts scheduler，以及作用于 attention/MLP projections 的 LoRA $r=16$、$\alpha=32$、dropout 0.1。各数据集差异如下：
...
\renewcommand{\arraystretch}{1.08}
\caption{Dataset-specific verifier settings. Intervals are measured in steps. All runs use a 12-epoch cap and canonical seed 42; checkpoints are selected by macro-F1 on validation (LIAR-RAW/RAWFC) or development (SciFact).}
\label{tab:verifier-settings}
\begin{tabularx}{\columnwidth}{@{}Y r r r@{}}
\toprule
\textbf{Dataset} & \textbf{Learning rate} & \textbf{Eval/save} & \textbf{Patience} \\
\midrule
LIAR-RAW & $2\times10^{-5}$ & 100 & 12 \\
RAWFC & $1\times10^{-5}$ & 50 & 8 \\
SciFact & $2\times10^{-5}$ & 100 & 12 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{LIAR-RAW and RAWFC Controlled Results}

\paragraph{Matched Internal Results}
...
\toprule
\textbf{Method} & \textbf{Comparison scope} & \textbf{LIAR-RAW P / R / F1 (FactLLaMA: LIAR)} & \textbf{RAWFC P / R / F1} \\
\midrule
\multicolumn{4}{@{}l}{\textbf{Same dataset family and original associated-report setting}} \\
...
\textbf{Ours} & Ministral-3-8B + LoRA; dataset-specific training & 38.38 / 34.83 / 35.40 & 65.26 / 65.01 / 65.08 \\
\multicolumn{4}{@{}l}{\textbf{Modified or external evidence setting}} \\
...
\multicolumn{4}{@{}l}{\textbf{Multi-stage / large-LLM contextual reference}} \\
